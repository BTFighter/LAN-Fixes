...
public static class PhotonNetwork
{
	...
	public static void SwitchToProtocol(ConnectionProtocol cp)
	{
		...
		public static bool ConnectUsingSettings(string gameVersion)
		{
			...
			{
				UnityEngine.Debug.Log("Using custom server settings from config.ini: " + customServerAddress + ":" + customServerPort);
				networkingPeer.IsUsingNameServer = false;
				networkingPeer.MasterServerAddress = ((customServerPort != 0) ? (customServerAddress + ":" + customServerPort) : customServerAddress);
		...
		private static int GetCustomServerPort()
		{
			try
			{
				...
				if (System.IO.File.Exists(configPath))
				{
					IniParser parser = new IniParser();
					parser.Load(configPath);
					return parser.GetIntValue("Server", "ServerPort", 0);
				}
			}
			catch (Exception e)
			{
				UnityEngine.Debug.LogWarning("Failed to read custom server port from LANSettings.ini: " + e.Message);
			}
			return 0;
		}
