...
internal class NetworkingPeer : LoadBalancingPeer, IPhotonPeerListener
{
	...
	protected internal string AppVersion => string.Format("{0}_{1}", PhotonNetwork.gameVersion, "1.85");
	...
	private string GetNameServerAddress()
	{
		...
		// Try to read custom server address from config file
		string configPath = Application.dataPath + "/../LANSettings.ini";
		if (File.Exists(configPath))
		{
			...
			try
			{
				...
				string customHost = iniParser.GetValue("Server", "ServerAddress", string.Empty);
				if (!string.IsNullOrEmpty(customHost))
				{
					nameServerHost = customHost;
				}
				customPort = iniParser.GetIntValue("Server", "Port", 0);
			}
			catch (Exception ex)
			{
				Debug.LogWarning("Failed to read LAN settings: " + ex.Message);
			}
		}
