...
internal class NetworkingPeer : LoadBalancingPeer, IPhotonPeerListener
{
	...
	protected internal string AppVersion => string.Format("{0}_{1}", PhotonNetwork.gameVersion, "1.81");
	...
	private string GetNameServerAddress()
	{
		...
		// Check for custom Photon server IP in config.ini
		string photonServerIp = "ns.exitgames.com"; // Default
		string configPath = System.IO.Path.Combine(Application.dataPath, "LANSettings.ini");
		if (!System.IO.File.Exists(configPath))
		{
			// Also check in same directory as executable
			configPath = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(Application.dataPath), "LANSettings.ini");
		}
		if (System.IO.File.Exists(configPath))
		{
			try
			{
				...
				string customIp = iniParser.GetValue("Server", "ServerAddress", "");
				if (!string.IsNullOrEmpty(customIp) && IsValidIPAddress(customIp))
				{
					photonServerIp = customIp;
				}
			}
			catch (Exception ex)
			{
				Debug.LogWarning("Failed to read LANSettings.ini: " + ex.Message);
			}
		}
		
		return string.Format("{0}{1}:{2}", arg, photonServerIp, value);
	}
	
	private bool IsValidIPAddress(string ip)
	{
		// Simple check for valid IPv4 address format
